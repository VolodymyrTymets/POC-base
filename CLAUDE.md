...
| typecheck | `pnpm --dir web -r exec tsc --noEmit && pnpm --dir web run typecheck` (the second covers `codegen.ts`, which `-r` skips — it's part of the workspace root, not a member package) |
...
- **W2** No test tooling exists yet for `web/` (deferred to KAN-11) — verification is typecheck + build + a real browser check until it lands.
