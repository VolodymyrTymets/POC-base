...
4. **No `any` in new code** — `unknown` plus narrowing. `eslint.config.mjs` currently sets `@typescript-eslint/no-explicit-any: 'off'` repo-wide; that is a leniency for existing code, not permission for new code. No non-null `!` and no `as` cast without a `// WHY:`. No `@ts-ignore`/`@ts-expect-error` without a human-approved `// WHY:` (rule B3).
...
7. `node:` prefix for Node built-ins. This package uses CommonJS (`sourceType: 'commonjs'` in `eslint.config.mjs`) — do not introduce ESM syntax that assumes a different module system without an ADR.
