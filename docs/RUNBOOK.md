...
| Postgres healthcheck in `docker-compose.yml` checks `pg_isready -U postgres -d trukkit`, but `api/.env.example` defaults to a database named `poc` | Leftover naming inconsistency (this repo was repurposed from a project called "Trukkit" into the generic `poc-base`). | Pick one database name and make `.env`/`docker-compose.yml` agree before relying on the Docker healthcheck. |
