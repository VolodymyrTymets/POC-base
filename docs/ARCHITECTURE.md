...
- `docker-compose.yml`'s `env_file: .env.development` resolves relative to the repo root, but the documented first-run step creates `api/.env.development`, not a root-level one — `docker compose up` fails with "env file not found" as committed. See `docs/RUNBOOK.md`. (The `api`/`worker` build-context landmine and the Postgres healthcheck db-name mismatch that used to be documented alongside this one are both fixed as of KAN-2, 2026-09-18 — see `docs/RUNBOOK.md`'s "Fixed by KAN-2" note.)
