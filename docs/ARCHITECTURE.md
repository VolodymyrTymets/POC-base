...
- `docker-compose.yml`'s Postgres healthcheck checks for a database named `trukkit` (`pg_isready -U postgres -d trukkit`) while `api/.env.example` defaults `DATABASE_URL` to a database named `poc` — these disagree; whichever is used, the other needs updating.
